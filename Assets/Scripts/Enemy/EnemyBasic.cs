...
public class EnemyBasic : ObjectBasic
{
    ...
    protected virtual void Detect()
    {
        ...
        {
            // ???Í≤? ?ú†Ïß? Í±∞Î¶¨Í∞? ?ñë?àò ?ù¥Í≥? ???Í≤üÏù¥ ?ú†Ïß?Í±∞Î¶¨Î≥¥Îã§ Î©?Î¶? ?ûà?ã§Î©? ???Í≤? ?ï¥?†ú
            if (0 <= enemyStats.detectionKeepDis && enemyStats.detectionKeepDis < enemyStatus.targetDis)
            {
                enemyStatus.enemyTarget = null; // ?îå?†à?ù¥?ñ¥Í∞? Í±∞Î¶¨ Ï°∞Ï†à?ïòÎ©? ?†Å?ì§ Ï£ΩÏùº ?àò ?ûàÍ≤?, ?ù¥Í±? ?ïà?ïòÎ©? ?†Å?ì§?ù¥ ?ÑàÎ¨? Î™∞Î†§????Ñú ?Çú?ù¥?èÑÍ∞? ?ÑàÎ¨? ?Üí?ïÑÏß?.
                return;
            }
            return;
        }
    ...
    {
        // Í≤ΩÏßÅÍ≥? Í≥µÍ≤© Ï§ëÏóê?äî ÏßÅÏ†ë ?ù¥?èô Î∂àÍ??
